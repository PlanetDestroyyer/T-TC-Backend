...
async def _run_deploy(deploy_id: str, repo_url: str, app_name: str, app_type: str, auto_restart: bool):
    ...
    try:
        ...
        step("Creating public URL...")
        tunnel_proc = _start_tunnel_process(app_name, port)
        tunnel_url = await _wait_for_tunnel_url(app_name)
        step(f"URL: {tunnel_url or 'unavailable'}", done=True)

        _pids[app_name] = {"app_pid": proc.pid, "tunnel_pid": tunnel_proc.pid}
...
def _launch_app(app: dict) -> subprocess.Popen | None:
    ...
    if t == "fastapi":
        cmd = ["python", "-m", "uvicorn", f"{_find_module(d)}:app", "--host", "0.0.0.0", "--port", str(p)]
    elif t == "flask":
...
async def _setup_tunnel(app_id: str, port: int):
    await asyncio.sleep(3)
    proc = _start_tunnel_process(app_id, port)
    if app_id in _pids:
        _pids[app_id]["tunnel_pid"] = proc.pid
