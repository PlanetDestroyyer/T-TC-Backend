...
def debug_ssh_setup():
    ...
    # Force-delete and regenerate ALL host keys — keys generated by ssh-keygen -A
    # inside proot may be silently corrupt (proot ENOSYS on getrandom/getentropy).
    for key_file in ["/etc/ssh/ssh_host_ed25519_key", "/etc/ssh/ssh_host_ed25519_key.pub",
    ...
    for key_type, key_file in [("ed25519", "/etc/ssh/ssh_host_ed25519_key"),
        ...
        # Fix permissions: private key 0600, public key 0644
        if r.returncode == 0:
