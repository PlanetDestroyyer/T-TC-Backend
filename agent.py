...
def debug_ssh_setup():
    """Install Dropbear SSH, generate host keys, set password, start server on port 2222.
    Uses Dropbear instead of OpenSSH — OpenSSH's seccomp-bpf sandbox conflicts with
    proot's ptrace interception, crashing every connection at the KEXINIT stage."""
    import subprocess, os, time
    results = {}

    # Install Dropbear if not present
    if not os.path.exists("/usr/sbin/dropbear"):
        r = subprocess.run(["apk", "add", "--no-cache", "--no-scripts", "dropbear"],
                           capture_output=True, text=True)
        results["apk_install"] = {"exit_code": r.returncode, "stdout": r.stdout[-500:]}
    else:
    ...
    # Kill any existing sshd/dropbear and start fresh
    subprocess.run(["pkill", "-f", "dropbear"], capture_output=True)
    subprocess.run(["pkill", "-f", "sshd"], capture_output=True)
    time.sleep(1)

    r = subprocess.Popen([
        "/usr/sbin/dropbear", "-F", "-E", "-p", "2222",
        "-r", "/etc/dropbear/dropbear_ed25519_hostkey",
        "-r", "/etc/dropbear/dropbear_rsa_hostkey",
    ])
    results["dropbear_started"] = {"pid": r.pid}
...
# ─────────────────────────────────────────────────────────────────────────────
# AI inference is handled on-device by llama.rn (React Native).
# No server-side AI endpoints needed.
# ─────────────────────────────────────────────────────────────────────────────
